defmodule LiveViewNativeTest.ClientProxy do
  @moduledoc false
  use GenServer
  ...
  alias LiveViewNativeTest.{ClientProxy, ViewTree, Element, View, Upload}
  ...
  ## Options
    ...
    * `:body` - the required string of markup for the document.
  ...
  def init(opts) do
    ...
    {id, session_token, static_token, redirect_url} =
      case Map.fetch(opts, :live_redirect) do
        ...
        :error ->
          [{id, session_token, static_token} | _] = ViewTree.find_live_views(root_markup)
    ...
    root_view = %ClientProxy{
      ...
      router: router,
      uri: URI.parse(url),
      child_statics: Map.delete(ViewTree.find_static_views(root_markup), id),
      topic: "lv:#{id}"
    }
    ...
    try do
      ...
      new_state =
        ...
        |> detect_added_or_removed_children(root_view, root_markup, [])

      send_caller(new_state, {:ok, build_client_view(root_view), ViewTree.to_markup(new_state.body)})
      {:ok, new_state}
    catch
  ...
  defp maybe_put_container(state, %{container: container}) do
    [tag, attrs] = container
    %{state | body: ViewTree.replace_root_container(state.body, tag, attrs)}
  end
      ...
      {:ok, pid} ->
        ...
        receive do
          {^ref, {:ok, %{rendered: rendered} = resp}} ->
            ...
            {%{view | pid: pid}, ViewTree.merge_diff(%{}, rendered), resp}
  ...
  defp start_supervised_channel(state, view, ref, url, redirect_url) do
    socket = %Phoenix.Socket{
      ...
      private: %{connect_info: Map.put_new(view.connect_info, :session, state.session), foobar: "asdf"},
      topic: view.topic,
    ...
    result =
      case topic_or_element do
        {topic, event, selector} ->
          ...
          cids =
            if selector do
              ViewTree.targets_from_selector(root(state, view), selector)
            else
              [nil]
            end
        ...
        %Element{} = element ->
          ...
          with {:ok, node} <- select_node(root, element),
               :ok <- maybe_enabled(type, node, element, view.client),
               {:ok, event_or_js, fallback} <- maybe_event(type, node, element),
               {:ok, dom_values} <- maybe_values(type, root, node, element, view.client) do
  ...
  def handle_call(:page_title, _from, %{page_title: :unset} = state) do
    state = %{state | page_title: root_page_title(state.body)}
  ...
  def handle_call(:html, _from, state) do
    {:reply, {:ok, {state.body, state.static_path}}, state}
  end
  ...
  defp put_view(state, %ClientProxy{pid: pid} = view, rendered) do
    ...
    patch_view(
      ...
      view,
      ViewTree.render_diff(rendered),
      rendered.streams
    )
  end

  defp patch_view(state, view, child_markup, streams) do
    case ViewTree.patch_id(view.id, state.body, child_markup, streams) do
      {new_markup, [_ | _] = will_destroy_cids} ->
        topic = view.topic
        state = %{state | body: new_markup}
        payload = %{"cids" => will_destroy_cids}

        push_with_callback(state, nil, view, "cids_will_destroy", payload, fn _, state ->
          still_there_cids = ViewTree.component_ids(view.id, state.body)
          ...
          state =
            push_with_callback(state, nil, view, "cids_destroyed", payload, fn reply, state ->
              ...
              {:noreply, update_in(state.views[topic].rendered, &ViewTree.drop_cids(&1, cids))}
            end)

          {:noreply, state}
        end)

      {new_markup, [] = _deleted_cids} ->
        %{state | body: new_markup}
    end
  end
  ...
  defp render_reply(reply, from, state) do
    case fetch_view_by_topic(state, reply.topic) do
      {:ok, view} ->
        GenServer.reply(from, {:ok, state.body |> ViewTree.inner_markup!(view.id) |> ViewTree.to_markup()})
        state
  ...
  defp merge_rendered(%{body: markup_before} = state, topic, %{} = diff) do
    {diff, state} =
      diff
    ...
    if diff == %{} do
      ...
      case fetch_view_by_topic(state, topic) do
        {:ok, view} ->
          rendered = ViewTree.merge_diff(view.rendered, diff)
          new_view = %ClientProxy{view | rendered: rendered}
          streams = ViewTree.extract_streams(rendered, rendered.streams)

          %{state | views: Map.update!(state.views, topic, fn _ -> new_view end)}
          |> patch_view(new_view, ViewTree.render_diff(rendered), streams)
          |> detect_added_or_removed_children(new_view, markup_before, streams)
  ...
  defp detect_added_or_removed_children(state, view, markup_before, streams) do
    new_state = recursive_detect_added_or_removed_children(state, view, markup_before, streams)
    {:ok, new_view} = fetch_view_by_topic(new_state, view.topic)

    ids_after =
      new_state.body
      |> ViewTree.reverse_filter(&ViewTree.attribute(&1, "data-phx-session"))
      |> ViewTree.all_attributes("id")
      |> MapSet.new()
  ...
  defp recursive_detect_added_or_removed_children(state, view, markup_before, streams) do
    state.body
    |> ViewTree.inner_markup!(view.id)
    |> ViewTree.find_live_views()
    |> Enum.reduce(state, fn {id, session, static}, acc ->
      case fetch_view_by_id(acc, id) do
        {:ok, view} ->
          streams = ViewTree.extract_streams(view.rendered, streams)
          patch_view(acc, view, ViewTree.inner_markup!(markup_before, view.id), streams)
        ...
        :error ->
          ...
          streams = ViewTree.extract_streams(rendered, streams)

          acc
          |> put_view(child_view, rendered)
          |> put_child(view, id, child_view.session_token)
          |> recursive_detect_added_or_removed_children(child_view, acc.body, streams)
      end
    end)
  end
  ...
  defp select_node(root, %Element{selector: selector, text_filter: nil}) do
    root
    |> ViewTree.child_nodes()
    |> ViewTree.maybe_one(selector)
  end
  ...
  defp select_node(root, {_, _, selector}) do
    if selector do
      root |> ViewTree.child_nodes() |> ViewTree.maybe_one(selector)
    else
      {:ok, root}
    end
  end
  ...
  defp select_node_by_text(root, nodes, text_filter, selector) do
    filtered_nodes = Enum.filter(nodes, &(ViewTree.to_text(&1) =~ text_filter))
  ...
  defp maybe_event(:hook, node, %Element{event: event} = element) do
    ...
    if ViewTree.attribute(node, "phx-hook") do
      if ViewTree.attribute(node, "id") do
        {:ok, event, []}
      else
        {:error, :invalid,
  ...
  defp maybe_event(:click, {"a", _, _} = node, element) do
    ...
    fallback =
      if to = ViewTree.attribute(node, "href") do
        case ViewTree.attribute(node, "data-phx-link") do
          "patch" ->
            [{:patch, proxy_topic(element), to}]

          "redirect" ->
            kind = ViewTree.attribute(node, "data-phx-link-state") || "push"
    ...
    cond do
      event = ViewTree.attribute(node, "phx-click") ->
        {:ok, event, fallback}
  ...
  defp maybe_event(type, node, element) when type in [:keyup, :keydown] do
    cond do
      event = ViewTree.attribute(node, "phx-#{type}") ->
        {:ok, event, []}

      event = ViewTree.attribute(node, "phx-window-#{type}") ->
        {:ok, event, []}
  ...
  defp maybe_event(type, node, element) do
    if event = ViewTree.attribute(node, "phx-#{type}") do
      {:ok, event, []}
    else
      {:error, :invalid,
      ...
      ["push", %{"event" => event} = args] ->
        ...
        {values, uploads} =
          case value do
            ...
            other -> {ViewTree.deep_merge(event_values, stringify(other, & &1)), nil}
          end

        js_targets = ViewTree.targets_from_selector(root, js_target_selector)
        node_targets = ViewTree.targets_from_node(root, node)
  ...
  defp maybe_enabled(_type, {tag, _, _}, %{form_data: form_data}, %{tags: tags})
       when tag != tags.form and form_data != nil do
    {:error, :invalid,
     "a form element was given but the selected node is not a form, got #{inspect(tag)}}"}
  end

  defp maybe_enabled(type, node, element, _client) do
    if ViewTree.attribute(node, "disabled") do
      {:error, :invalid,
  ...
  defp maybe_values(:hook, _root, _node, _element, _client), do: {:ok, %{}}

  defp maybe_values(type, root, {tag, attrs, _} = node, element, %{tags: tags})
      ...
      tag == tags.form ->
        ...
        named_inputs =
          case Enum.into(attrs, %{}) do
            %{"id" => id} -> Floki.find(root, "[#{tags.form}=#{id}]")
            _ -> []
          end

        named_btns = ViewTree.filter(named_inputs, fn node -> ViewTree.tag(node) == tags.button end)
        ...
        with {:ok, defaults} <- maybe_submitter(defaults, type, {node, all_inputs}, element),
          ...
          {:ok,
           defaults
           |> Query.decode_done()
           |> ViewTree.deep_merge(ViewTree.all_values(node))
           |> ViewTree.deep_merge(value)}
        else
  ...
  defp maybe_values(_type, _root, node, _element, _client) do
    {:ok, ViewTree.all_values(node)}
  end

  defp filtered_inputs(nodes) do
    ViewTree.filter(nodes, fn node ->
      ViewTree.tag(node) in ~w(input textarea select) and
        is_nil(ViewTree.attribute(node, "disabled"))
    end)
  end
  ...
  defp collect_submitter(node, form, element, defaults) do
    name = ViewTree.attribute(node, "name")
    ...
    cond do
      ...
      submitter?(node) and is_nil(ViewTree.attribute(node, "disabled")) ->
        {:ok, Plug.Conn.Query.decode_each({name, ViewTree.attribute(node, "value")}, defaults)}

      true ->
        {:error, :invalid,
         "could not find non-disabled submit input or button with name #{inspect(name)} within:\n\n" <>
           ViewTree.inspect_markup(ViewTree.all(form, "[name]"))}
    end
  end

  defp submitter?({"input", _, _} = node) do
    ViewTree.attribute(node, "type") == "submit"
  end

  defp submitter?({"button", _, _} = node) do
    ViewTree.attribute(node, "type") in ["submit", nil]
  end
  ...
  defp form_defaults(node, acc) do
    if name = ViewTree.attribute(node, "name") do
  ...
  defp form_defaults({"select", _, _} = node, name, acc) do
    options = ViewTree.filter(node, &(ViewTree.tag(&1) == "option"))
    ...
    all_selected =
      if ViewTree.attribute(node, "multiple") || multiple_display_size do
        Enum.filter(options, &ViewTree.attribute(&1, "selected"))
      else
        List.wrap(
          Enum.find(Enum.reverse(options), &ViewTree.attribute(&1, "selected")) ||
            Enum.find(options, &(!ViewTree.attribute(&1, "disabled")))
        )
      end

    Enum.reduce(all_selected, acc, fn selected, acc ->
      Plug.Conn.Query.decode_each({name, ViewTree.attribute(selected, "value")}, acc)
    end)
  end
  ...
  defp form_defaults({"input", _, _} = node, name, acc) do
    type = ViewTree.attribute(node, "type") || "text"
    value = ViewTree.attribute(node, "value") || ""

    cond do
      type in ["radio", "checkbox"] ->
        if ViewTree.attribute(node, "checked") do
  ...
  defp valid_display_size(node) do
    with size when not is_nil(size) <- ViewTree.attribute(node, "size"),
  ...
  defp fill_in_value(non_string_value, name, node) do
    ...
    {types, dom_values} =
      node
      |> ViewTree.filter(fn node ->
        ViewTree.attribute(node, "name") == name and is_nil(ViewTree.attribute(node, "disabled"))
      end)
      |> collect_values([], [])
      ...
      types == [] ->
        {:error, :invalid,
         "could not find non-disabled input, select or textarea with name #{inspect(name)} within:\n\n" <>
           ViewTree.inspect_markup(ViewTree.all(node, "[name]"))}
  ...
  defp collect_values([{"select", _, _} = node | nodes], types, values) do
    options =
      node
      |> ViewTree.filter(&(ViewTree.tag(&1) == "option"))
      |> Enum.map(&(ViewTree.attribute(&1, "value") || ""))

    if ViewTree.attribute(node, "multiple") do
  ...
  defp maybe_put_uploads(payload, state, view, %Upload{} = upload) do
    ...
    ref = ViewTree.attribute(node, "data-phx-upload-ref")
  ...
  defp root_page_title(root_markup) do
    case ViewTree.maybe_one(root_markup, "head > title") do
