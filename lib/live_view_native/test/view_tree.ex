defmodule LiveViewNativeTest.ViewTree do
  ...
  def maybe_one(view_tree, selector, type \\ :selector) do
    case all(view_tree, selector) do
      [node] ->
        {:ok, node}
  ...
  def all_attributes(view_tree, name), do: Floki.attribute(view_tree, name)

  def all_values({_, attributes, _}) do
    for {attr, value} <- attributes, key = value_key(attr), do: {key, value}, into: %{}
  end

  def inspect_markup(nodes) when is_list(nodes) do
    for view_tree_node <- nodes, into: "", do: inspect_markup(view_tree_node)
  end

  def inspect_markup(view_tree_node),
    do: "    " <> String.replace(to_markup(view_tree_node), "\n", "\n   ") <> "\n"
  ...
  def to_markup(view_tree), do: Floki.raw_html(view_tree)

  def to_text(view_tree), do: Floki.text(view_tree)

  # TODO: rewrite to use Floki.get_by_id/2
  # currently it does not raise when multiple elements are found
  def by_id!(view_tree, id) do
    case maybe_one(view_tree, "#" <> id) do
      {:ok, node} -> node
  ...
  def inner_markup!(view_tree, id), do: view_tree |> by_id!(id) |> child_nodes()

  def component_id(view_tree), do: Floki.attribute(view_tree, @phx_component) |> List.first()

  @doc """
  Find static information in the given markup tree.
  """
  def find_static_views(view_tree) do
    view_tree
    |> all("[#{@phx_static}]")
    |> Enum.into(%{}, fn node ->
  ...
  """
  def find_live_views(view_tree) do
    view_tree
  ...
  def patch_id(id, view_tree, inner_markup, streams) do
    cids_before = component_ids(id, view_tree)

    phx_update_tree =
      walk(inner_markup, fn node ->
        apply_phx_update(attribute(node, "phx-update"), view_tree, node, streams)
      end)

    new_markup =
      walk(view_tree, fn {tag, attrs, children} = node ->
    ...
    cids_after = component_ids(id, new_markup)
    {new_markup, cids_before -- cids_after}
  end

  def component_ids(id, view_tree) do
    by_id!(view_tree, id)
    |> Floki.children()
    |> Enum.reduce([], &traverse_component_ids/2)
  end

  def replace_root_container(container_markup, new_tag, attrs) do
    reserved_attrs = ~w(id data-phx-session data-phx-static data-phx-main)
    [{_container_tag, container_attrs_list, children}] = container_markup
  ...
  defp apply_phx_update(type, view_tree, {tag, attrs, appended_children} = node, _streams)
       when type in ["append", "prepend"] do
    container_id = attribute(node, "id")
    verify_phx_update_id!(type, container_id, node)
    children_before = apply_phx_update_children(view_tree, container_id)
  ...
  defp apply_phx_update("stream", view_tree, {tag, attrs, appended_children} = node, streams) do
    container_id = attribute(node, "id")
    verify_phx_update_id!("stream", container_id, node)
    children_before = apply_phx_update_children(view_tree, container_id)
    appended_children = appended_children
    # to ensure correct ViewTree patching, all elements must have an ID
    ...
    # now apply the ViewTree patching (this corresponds mainly to the appendChild in dom_patch.js)
    new_children =
        ...
        new_children =
          cond do
            ...
            is_nil(insert) && current_index ->
              # not a stream item, but already in the ViewTree -> update in place
  ...
  defp apply_phx_update("ignore", view_tree, node, _streams) do
    container_id = attribute(node, "id")
    ...
    {tag, attrs_before, children_before} =
      case by_id(view_tree, container_id) do
  ...
  defp apply_phx_update_children(view_tree, id) do
    case by_id(view_tree, id) do
  ...
  defp walk(view_tree, fun) when is_function(fun, 1) do
    Floki.traverse_and_update(view_tree, walk_fun(fun))
  end
  ...
  defp by_id(view_tree, id) do
    Floki.get_by_id(view_tree, id)
  end

  def parent_id(view_tree, child_id) do
    try do
      walk(view_tree, fn {tag, attrs, children} = node ->
