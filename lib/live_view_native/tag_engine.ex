defmodule LiveViewNative.TagEngine do
  ...
  def component(func, assigns, caller)
      when (is_function(func, 2) and is_list(assigns)) or is_map(assigns) do
    assigns =
      case assigns do
    ...
    case func.(assigns, interface) do
  ...
  def handle_body(state) do
    ...
    quote do
      require LiveViewNative.TagEngine
      unquote(ast)
    end
  end
  ...
  defp handle_token(
    ...
    ast =
      quote line: tag_meta.line do
        ...
        LiveViewNative.TagEngine.component(
          &(unquote(call) / 2),
          Map.put(unquote(assigns), :_interface, interface),
  ...
  defp handle_token({:close, :slot, slot_name, _tag_close_meta} = token, state) do
    ...
    ast =
      quote line: line do
        LiveViewNative.TagEngine.inner_block(unquote(slot_name), do: unquote(clauses))
      end
  ...
  defp build_component_assigns(type_component, attrs, line, tag_meta, state) do
    ...
    inner_block =
      quote line: line do
        LiveViewNative.TagEngine.inner_block(:inner_block, do: unquote(clauses))
      end
      ...
      {pattern, %{line: line}} ->
        quote line: line do
          ...
          quote line: line, generated: true do
            other ->
              LiveViewNative.TagEngine.__unmatched_let__!(
