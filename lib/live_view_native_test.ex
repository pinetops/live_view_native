defmodule LiveViewNativeTest do
  ...
  alias LiveViewNativeTest.{ClientProxy, ViewTree, Element, View, Upload, UploadClient}
  ...
  defp rendered_to_diff_string(rendered, socket) do
    {_, diff, _, _} = Diff.render(socket, rendered, Diff.new_fingerprints(), Diff.new_components())
  ...
  Children are returned in the order they appear in the rendered template body.
  ...
  @doc """
  Returns the string template body of the rendered view or element.
  ...
  def render(view_or_element) do
    view_or_element
    |> coerce_view_or_element()
    |> render_tree()
    |> ViewTree.to_markup()
  end

  defp coerce_view_or_element(%Phoenix.LiveViewTest.View{} = view),
    do: Map.put(view, :__struct__, View)

  defp coerce_view_or_element(%Phoenix.LiveViewTest.Element{} = element),
    do: Map.put(element, :__struct__, Element)

  defp coerce_view_or_element(view_or_element),
    do: view_or_element
  ...
  But it may be clearer to add an LVN template attribute to make the element easier to
  select.
  ...
  Given the form ViewTree selector, the upload name, and a list of maps of client metadata
  ...
  defp find_cid!(view, selector) do
    view_tree = view |> render() |> ViewTree.parse()

    with {:ok, form} <- ViewTree.maybe_one(view_tree, selector) do
      [cid | _] = ViewTree.targets_from_node(view_tree, form)
      cid
    else
  ...
  Or in the case of an error tuple:
      ...
      {:ok, view, body} = follow_redirect(result, conn, :gameboy)
  ...
  `{:ok, live_view, disconnected_body}` with the content of the new
